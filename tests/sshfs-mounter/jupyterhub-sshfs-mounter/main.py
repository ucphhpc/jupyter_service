...
parser.add_argument("--hub-url", dest="hub_url", default="http://127.0.0.1:8888")
parser.add_argument("--mount-url", dest="mount_url", default="/hub/mount")
parser.add_argument("--auth-url", dest="auth_url", default="/hub/login")
parser.add_argument("--spawn-url", dest="spawn_url", default="/hub/spawn")
parser.add_argument(
    "--docker-image", dest="docker_image", default="jupyterhub/singleuser:0.8.1"
)
...
def main(args):
    ...
    with requests.Session() as session:
        ...
        payload = {"dockerimage": args.docker_image}

        # Spawn
        session.post(args.hub_url + args.spawn_url, data=payload, headers=headers)
